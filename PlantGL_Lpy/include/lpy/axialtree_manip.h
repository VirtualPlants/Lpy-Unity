...
template<class Iterator>
Iterator endBracket(Iterator pos, Iterator string_end, IterMap(Iterator) * itermap = NULL, bool startingBeforePos = false)
{
  ...
  if (!startingBeforePos && pos->isLeftBracket()) {    
        typename IterMap(Iterator)::const_iterator mappedend = itermap->find(iter_to_hashable(pos, string_end));
...
template<class Iterator>
Iterator beginBracket(Iterator pos, Iterator string_begin, Iterator string_end, IterMap(Iterator)* itermap = NULL, bool startingAfterPos = false)
{ 
  ...
  while(pos != string_begin && (bracket>0 || !pos->isLeftBracket())){
    if(pos->isRightBracket()) {
        typename  IterMap(Iterator)::const_iterator mappedbegin = itermap->find(iter_to_hashable(pos, string_end));
...
template<class Iterator>
Iterator parent(Iterator pos, Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
  if( pos == string_begin ) return string_end;
  --pos;
...
template<class Iterator>
std::vector<Iterator>  children(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{ 
...
template<class Iterator>
std::vector<Iterator> lateral_children(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL) {
  std::vector<Iterator> result; 
  // current pos is the end of a branch
  if( (pos == string_end) || pos->isRightBracket()) return result;
  ++pos;
...
template<class Iterator>
Iterator direct_child(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
...
template<class Iterator>
Iterator direct_child_from_previous_pos(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
...
template<class Iterator>
std::vector<Iterator> roots(Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{ 
...
template<class Iterator>
Iterator complex(Iterator pos, int scale, Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
  if( pos == string_begin ) return string_end;
...
template<class Iterator>
std::vector<Iterator>  components(Iterator pos, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{ 
  std::vector<Iterator> result; 
  // current pos is the end of a branch
  if( (pos == string_end) || pos->isRightBracket()) return result;
...
template<class Iterator>
std::vector<Iterator>  components_at_scale(Iterator pos, int scale, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{ 
  std::vector<Iterator> result; 
  // current pos is the end of a branch
  if( (pos == string_end) || pos->isRightBracket()) return result;
...
template<class Iterator>
Iterator predecessor_at_scale(Iterator pos, int targetscale, Iterator string_begin, Iterator string_end, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
  if( pos == string_begin ) return string_end;
...
template<class Iterator>
Iterator successor_at_level(Iterator pos, int scale, Iterator string_end, bool fromPreviousPosition = false, const ConsiderFilterPtr filter = ConsiderFilterPtr(), IterMap(Iterator)* itermap = NULL)
{
  if( pos == string_end ) return string_end;
  if(fromPreviousPosition) pos = direct_child_from_previous_pos(pos, string_end, filter, itermap);
